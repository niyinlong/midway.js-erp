...
/**
 * DiscussChannelMember 服务类
 * 处理 discuss_channel_member 表的数据库操作
 */
@Provide()
export class DiscussChannelMemberService extends BaseService<DiscussChannelMember> {
  constructor(
    @InjectRepository(DiscussChannelMember)
    repository: Repository<DiscussChannelMember>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询DiscussChannelMember
   * 可以在此添加业务相关的查询方法
   */
