...
/**
 * WebsiteRoute 服务类
 * 处理 website_route 表的数据库操作
 */
@Provide()
export class WebsiteRouteService extends BaseService<WebsiteRoute> {
  constructor(
    @InjectRepository(WebsiteRoute)
    repository: Repository<WebsiteRoute>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询WebsiteRoute
   * 可以在此添加业务相关的查询方法
   */
