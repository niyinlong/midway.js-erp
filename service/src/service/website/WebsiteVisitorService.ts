...
/**
 * WebsiteVisitor 服务类
 * 处理 website_visitor 表的数据库操作
 */
@Provide()
export class WebsiteVisitorService extends BaseService<WebsiteVisitor> {
  constructor(
    @InjectRepository(WebsiteVisitor)
    repository: Repository<WebsiteVisitor>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询WebsiteVisitor
   * 可以在此添加业务相关的查询方法
   */
