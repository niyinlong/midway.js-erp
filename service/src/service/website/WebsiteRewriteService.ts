...
/**
 * WebsiteRewrite 服务类
 * 处理 website_rewrite 表的数据库操作
 */
@Provide()
export class WebsiteRewriteService extends BaseService<WebsiteRewrite> {
  constructor(
    @InjectRepository(WebsiteRewrite)
    repository: Repository<WebsiteRewrite>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询WebsiteRewrite
   * 可以在此添加业务相关的查询方法
   */
