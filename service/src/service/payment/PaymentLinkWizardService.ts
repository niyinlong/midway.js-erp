...
/**
 * PaymentLinkWizard 服务类
 * 处理 payment_link_wizard 表的数据库操作
 */
@Provide()
export class PaymentLinkWizardService extends BaseService<PaymentLinkWizard> {
  constructor(
    @InjectRepository(PaymentLinkWizard)
    repository: Repository<PaymentLinkWizard>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询PaymentLinkWizard
   * 可以在此添加业务相关的查询方法
   */
