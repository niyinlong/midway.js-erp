...
/**
 * PaymentRefundWizard 服务类
 * 处理 payment_refund_wizard 表的数据库操作
 */
@Provide()
export class PaymentRefundWizardService extends BaseService<PaymentRefundWizard> {
  constructor(
    @InjectRepository(PaymentRefundWizard)
    repository: Repository<PaymentRefundWizard>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询PaymentRefundWizard
   * 可以在此添加业务相关的查询方法
   */
