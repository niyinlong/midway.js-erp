...
/**
 * PaymentProvider 服务类
 * 处理 payment_provider 表的数据库操作
 */
@Provide()
export class PaymentProviderService extends BaseService<PaymentProvider> {
  constructor(
    @InjectRepository(PaymentProvider)
    repository: Repository<PaymentProvider>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询PaymentProvider
   * 可以在此添加业务相关的查询方法
   */
