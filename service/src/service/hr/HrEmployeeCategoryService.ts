...
/**
 * HrEmployeeCategory 服务类
 * 处理 hr_employee_category 表的数据库操作
 */
@Provide()
export class HrEmployeeCategoryService extends BaseService<HrEmployeeCategory> {
  constructor(
    @InjectRepository(HrEmployeeCategory)
    repository: Repository<HrEmployeeCategory>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询HrEmployeeCategory
   * 可以在此添加业务相关的查询方法
   */
