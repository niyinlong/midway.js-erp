...
/**
 * WebTourTourStep 服务类
 * 处理 web_tour_tour_step 表的数据库操作
 */
@Provide()
export class WebTourTourStepService extends BaseService<WebTourTourStep> {
  constructor(
    @InjectRepository(WebTourTourStep)
    repository: Repository<WebTourTourStep>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询WebTourTourStep
   * 可以在此添加业务相关的查询方法
   */
