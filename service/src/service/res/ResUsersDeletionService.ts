...
/**
 * ResUsersDeletion 服务类
 * 处理 res_users_deletion 表的数据库操作
 */
@Provide()
export class ResUsersDeletionService extends BaseService<ResUsersDeletion> {
  constructor(
    @InjectRepository(ResUsersDeletion)
    repository: Repository<ResUsersDeletion>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ResUsersDeletion
   * 可以在此添加业务相关的查询方法
   */
