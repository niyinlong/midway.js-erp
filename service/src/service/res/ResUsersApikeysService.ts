...
/**
 * ResUsersApikeys 服务类
 * 处理 res_users_apikeys 表的数据库操作
 */
@Provide()
export class ResUsersApikeysService extends BaseService<ResUsersApikeys> {
  constructor(
    @InjectRepository(ResUsersApikeys)
    repository: Repository<ResUsersApikeys>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ResUsersApikeys
   * 可以在此添加业务相关的查询方法
   */
