...
/**
 * ResPartnerCategory 服务类
 * 处理 res_partner_category 表的数据库操作
 */
@Provide()
export class ResPartnerCategoryService extends BaseService<ResPartnerCategory> {
  constructor(
    @InjectRepository(ResPartnerCategory)
    repository: Repository<ResPartnerCategory>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ResPartnerCategory
   * 可以在此添加业务相关的查询方法
   */
