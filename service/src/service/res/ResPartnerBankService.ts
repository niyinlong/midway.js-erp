...
/**
 * ResPartnerBank 服务类
 * 处理 res_partner_bank 表的数据库操作
 */
@Provide()
export class ResPartnerBankService extends BaseService<ResPartnerBank> {
  constructor(
    @InjectRepository(ResPartnerBank)
    repository: Repository<ResPartnerBank>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ResPartnerBank
   * 可以在此添加业务相关的查询方法
   */
