...
/**
 * ResCountryState 服务类
 * 处理 res_country_state 表的数据库操作
 */
@Provide()
export class ResCountryStateService extends BaseService<ResCountryState> {
  constructor(
    @InjectRepository(ResCountryState)
    repository: Repository<ResCountryState>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ResCountryState
   * 可以在此添加业务相关的查询方法
   */
