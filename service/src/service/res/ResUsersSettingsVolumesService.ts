...
/**
 * ResUsersSettingsVolumes 服务类
 * 处理 res_users_settings_volumes 表的数据库操作
 */
@Provide()
export class ResUsersSettingsVolumesService extends BaseService<ResUsersSettingsVolumes> {
  constructor(
    @InjectRepository(ResUsersSettingsVolumes)
    repository: Repository<ResUsersSettingsVolumes>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ResUsersSettingsVolumes
   * 可以在此添加业务相关的查询方法
   */
