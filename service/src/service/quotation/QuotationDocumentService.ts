...
/**
 * QuotationDocument 服务类
 * 处理 quotation_document 表的数据库操作
 */
@Provide()
export class QuotationDocumentService extends BaseService<QuotationDocument> {
  constructor(
    @InjectRepository(QuotationDocument)
    repository: Repository<QuotationDocument>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询QuotationDocument
   * 可以在此添加业务相关的查询方法
   */
