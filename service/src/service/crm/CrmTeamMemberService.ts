...
/**
 * CrmTeamMember 服务类
 * 处理 crm_team_member 表的数据库操作
 */
@Provide()
export class CrmTeamMemberService extends BaseService<CrmTeamMember> {
  constructor(
    @InjectRepository(CrmTeamMember)
    repository: Repository<CrmTeamMember>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询CrmTeamMember
   * 可以在此添加业务相关的查询方法
   */
