...
/**
 * CrmLeadLost 服务类
 * 处理 crm_lead_lost 表的数据库操作
 */
@Provide()
export class CrmLeadLostService extends BaseService<CrmLeadLost> {
  constructor(
    @InjectRepository(CrmLeadLost)
    repository: Repository<CrmLeadLost>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询CrmLeadLost
   * 可以在此添加业务相关的查询方法
   */
