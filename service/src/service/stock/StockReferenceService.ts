...
/**
 * StockReference 服务类
 * 处理 stock_reference 表的数据库操作
 */
@Provide()
export class StockReferenceService extends BaseService<StockReference> {
  constructor(
    @InjectRepository(StockReference)
    repository: Repository<StockReference>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询StockReference
   * 可以在此添加业务相关的查询方法
   */
