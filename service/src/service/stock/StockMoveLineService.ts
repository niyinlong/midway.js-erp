...
/**
 * StockMoveLine 服务类
 * 处理 stock_move_line 表的数据库操作
 */
@Provide()
export class StockMoveLineService extends BaseService<StockMoveLine> {
  constructor(
    @InjectRepository(StockMoveLine)
    repository: Repository<StockMoveLine>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询StockMoveLine
   * 可以在此添加业务相关的查询方法
   */
