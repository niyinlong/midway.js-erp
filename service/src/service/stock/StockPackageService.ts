...
/**
 * StockPackage 服务类
 * 处理 stock_package 表的数据库操作
 */
@Provide()
export class StockPackageService extends BaseService<StockPackage> {
  constructor(
    @InjectRepository(StockPackage)
    repository: Repository<StockPackage>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询StockPackage
   * 可以在此添加业务相关的查询方法
   */
