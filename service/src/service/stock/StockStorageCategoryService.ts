...
/**
 * StockStorageCategory 服务类
 * 处理 stock_storage_category 表的数据库操作
 */
@Provide()
export class StockStorageCategoryService extends BaseService<StockStorageCategory> {
  constructor(
    @InjectRepository(StockStorageCategory)
    repository: Repository<StockStorageCategory>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询StockStorageCategory
   * 可以在此添加业务相关的查询方法
   */
