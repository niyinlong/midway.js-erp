...
/**
 * StockWarehouse 服务类
 * 处理 stock_warehouse 表的数据库操作
 */
@Provide()
export class StockWarehouseService extends BaseService<StockWarehouse> {
  constructor(
    @InjectRepository(StockWarehouse)
    repository: Repository<StockWarehouse>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询StockWarehouse
   * 可以在此添加业务相关的查询方法
   */
