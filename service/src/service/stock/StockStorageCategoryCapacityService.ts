...
/**
 * StockStorageCategoryCapacity 服务类
 * 处理 stock_storage_category_capacity 表的数据库操作
 */
@Provide()
export class StockStorageCategoryCapacityService extends BaseService<StockStorageCategoryCapacity> {
  constructor(
    @InjectRepository(StockStorageCategoryCapacity)
    repository: Repository<StockStorageCategoryCapacity>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询StockStorageCategoryCapacity
   * 可以在此添加业务相关的查询方法
   */
