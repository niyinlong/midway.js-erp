...
/**
 * StockRouteMove 服务类
 * 处理 stock_route_move 表的数据库操作
 */
@Provide()
export class StockRouteMoveService extends BaseService<StockRouteMove> {
  constructor(
    @InjectRepository(StockRouteMove)
    repository: Repository<StockRouteMove>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询StockRouteMove
   * 可以在此添加业务相关的查询方法
   */
