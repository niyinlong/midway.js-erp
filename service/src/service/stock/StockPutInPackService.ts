...
/**
 * StockPutInPack 服务类
 * 处理 stock_put_in_pack 表的数据库操作
 */
@Provide()
export class StockPutInPackService extends BaseService<StockPutInPack> {
  constructor(
    @InjectRepository(StockPutInPack)
    repository: Repository<StockPutInPack>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询StockPutInPack
   * 可以在此添加业务相关的查询方法
   */
