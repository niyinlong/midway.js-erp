...
/**
 * StockPickingType 服务类
 * 处理 stock_picking_type 表的数据库操作
 */
@Provide()
export class StockPickingTypeService extends BaseService<StockPickingType> {
  constructor(
    @InjectRepository(StockPickingType)
    repository: Repository<StockPickingType>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询StockPickingType
   * 可以在此添加业务相关的查询方法
   */
