...
/**
 * ProductTaxesRel 服务类
 * 处理 product_taxes_rel 表的数据库操作
 */
@Provide()
export class ProductTaxesRelService extends BaseService<ProductTaxesRel> {
  constructor(
    @InjectRepository(ProductTaxesRel)
    repository: Repository<ProductTaxesRel>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductTaxesRel
   * 可以在此添加业务相关的查询方法
   */
