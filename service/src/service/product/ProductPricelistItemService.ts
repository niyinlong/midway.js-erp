...
/**
 * ProductPricelistItem 服务类
 * 处理 product_pricelist_item 表的数据库操作
 */
@Provide()
export class ProductPricelistItemService extends BaseService<ProductPricelistItem> {
  constructor(
    @InjectRepository(ProductPricelistItem)
    repository: Repository<ProductPricelistItem>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductPricelistItem
   * 可以在此添加业务相关的查询方法
   */
