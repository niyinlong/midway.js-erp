...
/**
 * ProductSupplierTaxesRel 服务类
 * 处理 product_supplier_taxes_rel 表的数据库操作
 */
@Provide()
export class ProductSupplierTaxesRelService extends BaseService<ProductSupplierTaxesRel> {
  constructor(
    @InjectRepository(ProductSupplierTaxesRel)
    repository: Repository<ProductSupplierTaxesRel>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductSupplierTaxesRel
   * 可以在此添加业务相关的查询方法
   */
