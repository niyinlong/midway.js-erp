...
/**
 * ProductLabelLayout 服务类
 * 处理 product_label_layout 表的数据库操作
 */
@Provide()
export class ProductLabelLayoutService extends BaseService<ProductLabelLayout> {
  constructor(
    @InjectRepository(ProductLabelLayout)
    repository: Repository<ProductLabelLayout>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductLabelLayout
   * 可以在此添加业务相关的查询方法
   */
