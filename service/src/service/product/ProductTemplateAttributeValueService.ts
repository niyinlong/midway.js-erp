...
/**
 * ProductTemplateAttributeValue 服务类
 * 处理 product_template_attribute_value 表的数据库操作
 */
@Provide()
export class ProductTemplateAttributeValueService extends BaseService<ProductTemplateAttributeValue> {
  constructor(
    @InjectRepository(ProductTemplateAttributeValue)
    repository: Repository<ProductTemplateAttributeValue>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductTemplateAttributeValue
   * 可以在此添加业务相关的查询方法
   */
