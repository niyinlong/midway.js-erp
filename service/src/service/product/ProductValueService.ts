...
/**
 * ProductValue 服务类
 * 处理 product_value 表的数据库操作
 */
@Provide()
export class ProductValueService extends BaseService<ProductValue> {
  constructor(
    @InjectRepository(ProductValue)
    repository: Repository<ProductValue>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductValue
   * 可以在此添加业务相关的查询方法
   */
