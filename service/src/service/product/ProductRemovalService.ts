...
/**
 * ProductRemoval 服务类
 * 处理 product_removal 表的数据库操作
 */
@Provide()
export class ProductRemovalService extends BaseService<ProductRemoval> {
  constructor(
    @InjectRepository(ProductRemoval)
    repository: Repository<ProductRemoval>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductRemoval
   * 可以在此添加业务相关的查询方法
   */
