...
/**
 * ProductProduct 服务类
 * 处理 product_product 表的数据库操作
 */
@Provide()
export class ProductProductService extends BaseService<ProductProduct> {
  constructor(
    @InjectRepository(ProductProduct)
    repository: Repository<ProductProduct>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductProduct
   * 可以在此添加业务相关的查询方法
   */
