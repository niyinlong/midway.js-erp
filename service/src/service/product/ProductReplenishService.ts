...
/**
 * ProductReplenish 服务类
 * 处理 product_replenish 表的数据库操作
 */
@Provide()
export class ProductReplenishService extends BaseService<ProductReplenish> {
  constructor(
    @InjectRepository(ProductReplenish)
    repository: Repository<ProductReplenish>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductReplenish
   * 可以在此添加业务相关的查询方法
   */
