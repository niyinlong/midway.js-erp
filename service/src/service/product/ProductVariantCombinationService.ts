...
/**
 * ProductVariantCombination 服务类
 * 处理 product_variant_combination 表的数据库操作
 */
@Provide()
export class ProductVariantCombinationService extends BaseService<ProductVariantCombination> {
  constructor(
    @InjectRepository(ProductVariantCombination)
    repository: Repository<ProductVariantCombination>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductVariantCombination
   * 可以在此添加业务相关的查询方法
   */
