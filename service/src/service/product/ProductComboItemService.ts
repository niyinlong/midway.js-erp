...
/**
 * ProductComboItem 服务类
 * 处理 product_combo_item 表的数据库操作
 */
@Provide()
export class ProductComboItemService extends BaseService<ProductComboItem> {
  constructor(
    @InjectRepository(ProductComboItem)
    repository: Repository<ProductComboItem>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductComboItem
   * 可以在此添加业务相关的查询方法
   */
