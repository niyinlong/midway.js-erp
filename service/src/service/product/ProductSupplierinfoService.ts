...
/**
 * ProductSupplierinfo 服务类
 * 处理 product_supplierinfo 表的数据库操作
 */
@Provide()
export class ProductSupplierinfoService extends BaseService<ProductSupplierinfo> {
  constructor(
    @InjectRepository(ProductSupplierinfo)
    repository: Repository<ProductSupplierinfo>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductSupplierinfo
   * 可以在此添加业务相关的查询方法
   */
