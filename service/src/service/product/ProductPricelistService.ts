...
/**
 * ProductPricelist 服务类
 * 处理 product_pricelist 表的数据库操作
 */
@Provide()
export class ProductPricelistService extends BaseService<ProductPricelist> {
  constructor(
    @InjectRepository(ProductPricelist)
    repository: Repository<ProductPricelist>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProductPricelist
   * 可以在此添加业务相关的查询方法
   */
