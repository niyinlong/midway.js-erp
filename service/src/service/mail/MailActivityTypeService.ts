...
/**
 * MailActivityType 服务类
 * 处理 mail_activity_type 表的数据库操作
 */
@Provide()
export class MailActivityTypeService extends BaseService<MailActivityType> {
  constructor(
    @InjectRepository(MailActivityType)
    repository: Repository<MailActivityType>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailActivityType
   * 可以在此添加业务相关的查询方法
   */
