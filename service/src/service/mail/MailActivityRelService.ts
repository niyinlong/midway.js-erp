...
/**
 * MailActivityRel 服务类
 * 处理 mail_activity_rel 表的数据库操作
 */
@Provide()
export class MailActivityRelService extends BaseService<MailActivityRel> {
  constructor(
    @InjectRepository(MailActivityRel)
    repository: Repository<MailActivityRel>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailActivityRel
   * 可以在此添加业务相关的查询方法
   */
