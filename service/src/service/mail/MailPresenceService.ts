...
/**
 * MailPresence 服务类
 * 处理 mail_presence 表的数据库操作
 */
@Provide()
export class MailPresenceService extends BaseService<MailPresence> {
  constructor(
    @InjectRepository(MailPresence)
    repository: Repository<MailPresence>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailPresence
   * 可以在此添加业务相关的查询方法
   */
