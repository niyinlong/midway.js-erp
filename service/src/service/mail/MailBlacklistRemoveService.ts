...
/**
 * MailBlacklistRemove 服务类
 * 处理 mail_blacklist_remove 表的数据库操作
 */
@Provide()
export class MailBlacklistRemoveService extends BaseService<MailBlacklistRemove> {
  constructor(
    @InjectRepository(MailBlacklistRemove)
    repository: Repository<MailBlacklistRemove>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailBlacklistRemove
   * 可以在此添加业务相关的查询方法
   */
