...
/**
 * MailMessageSchedule 服务类
 * 处理 mail_message_schedule 表的数据库操作
 */
@Provide()
export class MailMessageScheduleService extends BaseService<MailMessageSchedule> {
  constructor(
    @InjectRepository(MailMessageSchedule)
    repository: Repository<MailMessageSchedule>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailMessageSchedule
   * 可以在此添加业务相关的查询方法
   */
