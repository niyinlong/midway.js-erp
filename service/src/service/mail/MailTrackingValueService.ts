...
/**
 * MailTrackingValue 服务类
 * 处理 mail_tracking_value 表的数据库操作
 */
@Provide()
export class MailTrackingValueService extends BaseService<MailTrackingValue> {
  constructor(
    @InjectRepository(MailTrackingValue)
    repository: Repository<MailTrackingValue>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailTrackingValue
   * 可以在此添加业务相关的查询方法
   */
