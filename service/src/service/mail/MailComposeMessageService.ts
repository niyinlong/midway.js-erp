...
/**
 * MailComposeMessage 服务类
 * 处理 mail_compose_message 表的数据库操作
 */
@Provide()
export class MailComposeMessageService extends BaseService<MailComposeMessage> {
  constructor(
    @InjectRepository(MailComposeMessage)
    repository: Repository<MailComposeMessage>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailComposeMessage
   * 可以在此添加业务相关的查询方法
   */
