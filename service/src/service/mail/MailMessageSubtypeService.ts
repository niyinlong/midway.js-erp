...
/**
 * MailMessageSubtype 服务类
 * 处理 mail_message_subtype 表的数据库操作
 */
@Provide()
export class MailMessageSubtypeService extends BaseService<MailMessageSubtype> {
  constructor(
    @InjectRepository(MailMessageSubtype)
    repository: Repository<MailMessageSubtype>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailMessageSubtype
   * 可以在此添加业务相关的查询方法
   */
