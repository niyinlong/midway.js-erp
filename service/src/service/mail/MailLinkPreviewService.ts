...
/**
 * MailLinkPreview 服务类
 * 处理 mail_link_preview 表的数据库操作
 */
@Provide()
export class MailLinkPreviewService extends BaseService<MailLinkPreview> {
  constructor(
    @InjectRepository(MailLinkPreview)
    repository: Repository<MailLinkPreview>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailLinkPreview
   * 可以在此添加业务相关的查询方法
   */
