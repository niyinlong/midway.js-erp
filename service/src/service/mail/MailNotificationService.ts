...
/**
 * MailNotification 服务类
 * 处理 mail_notification 表的数据库操作
 */
@Provide()
export class MailNotificationService extends BaseService<MailNotification> {
  constructor(
    @InjectRepository(MailNotification)
    repository: Repository<MailNotification>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailNotification
   * 可以在此添加业务相关的查询方法
   */
