...
/**
 * MailIceServer 服务类
 * 处理 mail_ice_server 表的数据库操作
 */
@Provide()
export class MailIceServerService extends BaseService<MailIceServer> {
  constructor(
    @InjectRepository(MailIceServer)
    repository: Repository<MailIceServer>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailIceServer
   * 可以在此添加业务相关的查询方法
   */
