...
/**
 * MailTemplatePreview 服务类
 * 处理 mail_template_preview 表的数据库操作
 */
@Provide()
export class MailTemplatePreviewService extends BaseService<MailTemplatePreview> {
  constructor(
    @InjectRepository(MailTemplatePreview)
    repository: Repository<MailTemplatePreview>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailTemplatePreview
   * 可以在此添加业务相关的查询方法
   */
