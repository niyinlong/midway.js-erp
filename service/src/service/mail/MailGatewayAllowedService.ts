...
/**
 * MailGatewayAllowed 服务类
 * 处理 mail_gateway_allowed 表的数据库操作
 */
@Provide()
export class MailGatewayAllowedService extends BaseService<MailGatewayAllowed> {
  constructor(
    @InjectRepository(MailGatewayAllowed)
    repository: Repository<MailGatewayAllowed>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询MailGatewayAllowed
   * 可以在此添加业务相关的查询方法
   */
