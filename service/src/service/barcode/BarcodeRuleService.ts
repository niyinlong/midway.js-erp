...
/**
 * BarcodeRule 服务类
 * 处理 barcode_rule 表的数据库操作
 */
@Provide()
export class BarcodeRuleService extends BaseService<BarcodeRule> {
  constructor(
    @InjectRepository(BarcodeRule)
    repository: Repository<BarcodeRule>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询BarcodeRule
   * 可以在此添加业务相关的查询方法
   */
