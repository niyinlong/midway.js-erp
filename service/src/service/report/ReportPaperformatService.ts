...
/**
 * ReportPaperformat 服务类
 * 处理 report_paperformat 表的数据库操作
 */
@Provide()
export class ReportPaperformatService extends BaseService<ReportPaperformat> {
  constructor(
    @InjectRepository(ReportPaperformat)
    repository: Repository<ReportPaperformat>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ReportPaperformat
   * 可以在此添加业务相关的查询方法
   */
