...
/**
 * AccountPaymentTermLine 服务类
 * 处理 account_payment_term_line 表的数据库操作
 */
@Provide()
export class AccountPaymentTermLineService extends BaseService<AccountPaymentTermLine> {
  constructor(
    @InjectRepository(AccountPaymentTermLine)
    repository: Repository<AccountPaymentTermLine>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountPaymentTermLine
   * 可以在此添加业务相关的查询方法
   */
