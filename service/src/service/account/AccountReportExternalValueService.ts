...
/**
 * AccountReportExternalValue 服务类
 * 处理 account_report_external_value 表的数据库操作
 */
@Provide()
export class AccountReportExternalValueService extends BaseService<AccountReportExternalValue> {
  constructor(
    @InjectRepository(AccountReportExternalValue)
    repository: Repository<AccountReportExternalValue>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountReportExternalValue
   * 可以在此添加业务相关的查询方法
   */
