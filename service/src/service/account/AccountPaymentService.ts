...
/**
 * AccountPayment 服务类
 * 处理 account_payment 表的数据库操作
 */
@Provide()
export class AccountPaymentService extends BaseService<AccountPayment> {
  constructor(
    @InjectRepository(AccountPayment)
    repository: Repository<AccountPayment>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountPayment
   * 可以在此添加业务相关的查询方法
   */
