...
/**
 * AccountPaymentRegister 服务类
 * 处理 account_payment_register 表的数据库操作
 */
@Provide()
export class AccountPaymentRegisterService extends BaseService<AccountPaymentRegister> {
  constructor(
    @InjectRepository(AccountPaymentRegister)
    repository: Repository<AccountPaymentRegister>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountPaymentRegister
   * 可以在此添加业务相关的查询方法
   */
