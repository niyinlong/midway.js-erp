...
/**
 * AccountMoveLine 服务类
 * 处理 account_move_line 表的数据库操作
 */
@Provide()
export class AccountMoveLineService extends BaseService<AccountMoveLine> {
  constructor(
    @InjectRepository(AccountMoveLine)
    repository: Repository<AccountMoveLine>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountMoveLine
   * 可以在此添加业务相关的查询方法
   */
