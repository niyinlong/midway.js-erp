...
/**
 * AccountPaymentTerm 服务类
 * 处理 account_payment_term 表的数据库操作
 */
@Provide()
export class AccountPaymentTermService extends BaseService<AccountPaymentTerm> {
  constructor(
    @InjectRepository(AccountPaymentTerm)
    repository: Repository<AccountPaymentTerm>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountPaymentTerm
   * 可以在此添加业务相关的查询方法
   */
