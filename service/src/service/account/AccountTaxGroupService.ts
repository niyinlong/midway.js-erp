...
/**
 * AccountTaxGroup 服务类
 * 处理 account_tax_group 表的数据库操作
 */
@Provide()
export class AccountTaxGroupService extends BaseService<AccountTaxGroup> {
  constructor(
    @InjectRepository(AccountTaxGroup)
    repository: Repository<AccountTaxGroup>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountTaxGroup
   * 可以在此添加业务相关的查询方法
   */
