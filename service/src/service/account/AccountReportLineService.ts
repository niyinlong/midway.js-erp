...
/**
 * AccountReportLine 服务类
 * 处理 account_report_line 表的数据库操作
 */
@Provide()
export class AccountReportLineService extends BaseService<AccountReportLine> {
  constructor(
    @InjectRepository(AccountReportLine)
    repository: Repository<AccountReportLine>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountReportLine
   * 可以在此添加业务相关的查询方法
   */
