...
/**
 * AccountPaymentMethodLine 服务类
 * 处理 account_payment_method_line 表的数据库操作
 */
@Provide()
export class AccountPaymentMethodLineService extends BaseService<AccountPaymentMethodLine> {
  constructor(
    @InjectRepository(AccountPaymentMethodLine)
    repository: Repository<AccountPaymentMethodLine>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountPaymentMethodLine
   * 可以在此添加业务相关的查询方法
   */
