...
/**
 * AccountPaymentMethod 服务类
 * 处理 account_payment_method 表的数据库操作
 */
@Provide()
export class AccountPaymentMethodService extends BaseService<AccountPaymentMethod> {
  constructor(
    @InjectRepository(AccountPaymentMethod)
    repository: Repository<AccountPaymentMethod>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountPaymentMethod
   * 可以在此添加业务相关的查询方法
   */
