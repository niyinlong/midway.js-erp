...
/**
 * AccountReportExpression 服务类
 * 处理 account_report_expression 表的数据库操作
 */
@Provide()
export class AccountReportExpressionService extends BaseService<AccountReportExpression> {
  constructor(
    @InjectRepository(AccountReportExpression)
    repository: Repository<AccountReportExpression>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountReportExpression
   * 可以在此添加业务相关的查询方法
   */
