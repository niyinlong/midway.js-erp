...
/**
 * AccountBankStatement 服务类
 * 处理 account_bank_statement 表的数据库操作
 */
@Provide()
export class AccountBankStatementService extends BaseService<AccountBankStatement> {
  constructor(
    @InjectRepository(AccountBankStatement)
    repository: Repository<AccountBankStatement>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountBankStatement
   * 可以在此添加业务相关的查询方法
   */
