...
/**
 * AccountPaymentRegisterMoveLineRel 服务类
 * 处理 account_payment_register_move_line_rel 表的数据库操作
 */
@Provide()
export class AccountPaymentRegisterMoveLineRelService extends BaseService<AccountPaymentRegisterMoveLineRel> {
  constructor(
    @InjectRepository(AccountPaymentRegisterMoveLineRel)
    repository: Repository<AccountPaymentRegisterMoveLineRel>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountPaymentRegisterMoveLineRel
   * 可以在此添加业务相关的查询方法
   */
