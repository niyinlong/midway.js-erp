...
/**
 * AccountReportColumn 服务类
 * 处理 account_report_column 表的数据库操作
 */
@Provide()
export class AccountReportColumnService extends BaseService<AccountReportColumn> {
  constructor(
    @InjectRepository(AccountReportColumn)
    repository: Repository<AccountReportColumn>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AccountReportColumn
   * 可以在此添加业务相关的查询方法
   */
