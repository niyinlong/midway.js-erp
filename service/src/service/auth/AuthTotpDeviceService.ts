...
/**
 * AuthTotpDevice 服务类
 * 处理 auth_totp_device 表的数据库操作
 */
@Provide()
export class AuthTotpDeviceService extends BaseService<AuthTotpDevice> {
  constructor(
    @InjectRepository(AuthTotpDevice)
    repository: Repository<AuthTotpDevice>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AuthTotpDevice
   * 可以在此添加业务相关的查询方法
   */
