...
/**
 * AuthPasskeyKey 服务类
 * 处理 auth_passkey_key 表的数据库操作
 */
@Provide()
export class AuthPasskeyKeyService extends BaseService<AuthPasskeyKey> {
  constructor(
    @InjectRepository(AuthPasskeyKey)
    repository: Repository<AuthPasskeyKey>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AuthPasskeyKey
   * 可以在此添加业务相关的查询方法
   */
