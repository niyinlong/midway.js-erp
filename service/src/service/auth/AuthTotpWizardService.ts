...
/**
 * AuthTotpWizard 服务类
 * 处理 auth_totp_wizard 表的数据库操作
 */
@Provide()
export class AuthTotpWizardService extends BaseService<AuthTotpWizard> {
  constructor(
    @InjectRepository(AuthTotpWizard)
    repository: Repository<AuthTotpWizard>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询AuthTotpWizard
   * 可以在此添加业务相关的查询方法
   */
