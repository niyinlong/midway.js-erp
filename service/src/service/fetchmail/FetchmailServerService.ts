...
/**
 * FetchmailServer 服务类
 * 处理 fetchmail_server 表的数据库操作
 */
@Provide()
export class FetchmailServerService extends BaseService<FetchmailServer> {
  constructor(
    @InjectRepository(FetchmailServer)
    repository: Repository<FetchmailServer>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询FetchmailServer
   * 可以在此添加业务相关的查询方法
   */
