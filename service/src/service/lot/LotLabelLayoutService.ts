...
/**
 * LotLabelLayout 服务类
 * 处理 lot_label_layout 表的数据库操作
 */
@Provide()
export class LotLabelLayoutService extends BaseService<LotLabelLayout> {
  constructor(
    @InjectRepository(LotLabelLayout)
    repository: Repository<LotLabelLayout>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询LotLabelLayout
   * 可以在此添加业务相关的查询方法
   */
