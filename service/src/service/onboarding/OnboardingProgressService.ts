...
/**
 * OnboardingProgress 服务类
 * 处理 onboarding_progress 表的数据库操作
 */
@Provide()
export class OnboardingProgressService extends BaseService<OnboardingProgress> {
  constructor(
    @InjectRepository(OnboardingProgress)
    repository: Repository<OnboardingProgress>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询OnboardingProgress
   * 可以在此添加业务相关的查询方法
   */
