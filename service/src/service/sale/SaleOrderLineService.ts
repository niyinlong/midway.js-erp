...
/**
 * SaleOrderLine 服务类
 * 处理 sale_order_line 表的数据库操作
 */
@Provide()
export class SaleOrderLineService extends BaseService<SaleOrderLine> {
  constructor(
    @InjectRepository(SaleOrderLine)
    repository: Repository<SaleOrderLine>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询SaleOrderLine
   * 可以在此添加业务相关的查询方法
   */
