...
/**
 * SaleOrderTagRel 服务类
 * 处理 sale_order_tag_rel 表的数据库操作
 */
@Provide()
export class SaleOrderTagRelService extends BaseService<SaleOrderTagRel> {
  constructor(
    @InjectRepository(SaleOrderTagRel)
    repository: Repository<SaleOrderTagRel>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询SaleOrderTagRel
   * 可以在此添加业务相关的查询方法
   */
