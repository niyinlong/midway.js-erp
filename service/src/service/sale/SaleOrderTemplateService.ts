...
/**
 * SaleOrderTemplate 服务类
 * 处理 sale_order_template 表的数据库操作
 */
@Provide()
export class SaleOrderTemplateService extends BaseService<SaleOrderTemplate> {
  constructor(
    @InjectRepository(SaleOrderTemplate)
    repository: Repository<SaleOrderTemplate>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询SaleOrderTemplate
   * 可以在此添加业务相关的查询方法
   */
