...
/**
 * SaleAdvancePaymentInv 服务类
 * 处理 sale_advance_payment_inv 表的数据库操作
 */
@Provide()
export class SaleAdvancePaymentInvService extends BaseService<SaleAdvancePaymentInv> {
  constructor(
    @InjectRepository(SaleAdvancePaymentInv)
    repository: Repository<SaleAdvancePaymentInv>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询SaleAdvancePaymentInv
   * 可以在此添加业务相关的查询方法
   */
