...
/**
 * PhoneBlacklist 服务类
 * 处理 phone_blacklist 表的数据库操作
 */
@Provide()
export class PhoneBlacklistService extends BaseService<PhoneBlacklist> {
  constructor(
    @InjectRepository(PhoneBlacklist)
    repository: Repository<PhoneBlacklist>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询PhoneBlacklist
   * 可以在此添加业务相关的查询方法
   */
