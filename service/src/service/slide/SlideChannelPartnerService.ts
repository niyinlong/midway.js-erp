...
/**
 * SlideChannelPartner 服务类
 * 处理 slide_channel_partner 表的数据库操作
 */
@Provide()
export class SlideChannelPartnerService extends BaseService<SlideChannelPartner> {
  constructor(
    @InjectRepository(SlideChannelPartner)
    repository: Repository<SlideChannelPartner>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询SlideChannelPartner
   * 可以在此添加业务相关的查询方法
   */
