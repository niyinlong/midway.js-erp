...
/**
 * SlideChannelTag 服务类
 * 处理 slide_channel_tag 表的数据库操作
 */
@Provide()
export class SlideChannelTagService extends BaseService<SlideChannelTag> {
  constructor(
    @InjectRepository(SlideChannelTag)
    repository: Repository<SlideChannelTag>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询SlideChannelTag
   * 可以在此添加业务相关的查询方法
   */
