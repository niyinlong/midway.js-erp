...
/**
 * SnailmailLetter 服务类
 * 处理 snailmail_letter 表的数据库操作
 */
@Provide()
export class SnailmailLetterService extends BaseService<SnailmailLetter> {
  constructor(
    @InjectRepository(SnailmailLetter)
    repository: Repository<SnailmailLetter>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询SnailmailLetter
   * 可以在此添加业务相关的查询方法
   */
