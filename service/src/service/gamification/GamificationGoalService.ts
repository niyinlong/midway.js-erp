...
/**
 * GamificationGoal 服务类
 * 处理 gamification_goal 表的数据库操作
 */
@Provide()
export class GamificationGoalService extends BaseService<GamificationGoal> {
  constructor(
    @InjectRepository(GamificationGoal)
    repository: Repository<GamificationGoal>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询GamificationGoal
   * 可以在此添加业务相关的查询方法
   */
