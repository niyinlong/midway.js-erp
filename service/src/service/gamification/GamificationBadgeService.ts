...
/**
 * GamificationBadge 服务类
 * 处理 gamification_badge 表的数据库操作
 */
@Provide()
export class GamificationBadgeService extends BaseService<GamificationBadge> {
  constructor(
    @InjectRepository(GamificationBadge)
    repository: Repository<GamificationBadge>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询GamificationBadge
   * 可以在此添加业务相关的查询方法
   */
