...
/**
 * ResourceCalendarAttendance 服务类
 * 处理 resource_calendar_attendance 表的数据库操作
 */
@Provide()
export class ResourceCalendarAttendanceService extends BaseService<ResourceCalendarAttendance> {
  constructor(
    @InjectRepository(ResourceCalendarAttendance)
    repository: Repository<ResourceCalendarAttendance>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ResourceCalendarAttendance
   * 可以在此添加业务相关的查询方法
   */
