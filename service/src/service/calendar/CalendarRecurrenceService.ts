...
/**
 * CalendarRecurrence 服务类
 * 处理 calendar_recurrence 表的数据库操作
 */
@Provide()
export class CalendarRecurrenceService extends BaseService<CalendarRecurrence> {
  constructor(
    @InjectRepository(CalendarRecurrence)
    repository: Repository<CalendarRecurrence>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询CalendarRecurrence
   * 可以在此添加业务相关的查询方法
   */
