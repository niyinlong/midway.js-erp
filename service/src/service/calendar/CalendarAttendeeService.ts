...
/**
 * CalendarAttendee 服务类
 * 处理 calendar_attendee 表的数据库操作
 */
@Provide()
export class CalendarAttendeeService extends BaseService<CalendarAttendee> {
  constructor(
    @InjectRepository(CalendarAttendee)
    repository: Repository<CalendarAttendee>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询CalendarAttendee
   * 可以在此添加业务相关的查询方法
   */
