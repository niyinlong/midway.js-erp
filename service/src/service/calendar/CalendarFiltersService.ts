...
/**
 * CalendarFilters 服务类
 * 处理 calendar_filters 表的数据库操作
 */
@Provide()
export class CalendarFiltersService extends BaseService<CalendarFilters> {
  constructor(
    @InjectRepository(CalendarFilters)
    repository: Repository<CalendarFilters>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询CalendarFilters
   * 可以在此添加业务相关的查询方法
   */
