...
/**
 * DigestDigest 服务类
 * 处理 digest_digest 表的数据库操作
 */
@Provide()
export class DigestDigestService extends BaseService<DigestDigest> {
  constructor(
    @InjectRepository(DigestDigest)
    repository: Repository<DigestDigest>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询DigestDigest
   * 可以在此添加业务相关的查询方法
   */
