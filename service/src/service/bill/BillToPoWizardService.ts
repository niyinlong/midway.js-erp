...
/**
 * BillToPoWizard 服务类
 * 处理 bill_to_po_wizard 表的数据库操作
 */
@Provide()
export class BillToPoWizardService extends BaseService<BillToPoWizard> {
  constructor(
    @InjectRepository(BillToPoWizard)
    repository: Repository<BillToPoWizard>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询BillToPoWizard
   * 可以在此添加业务相关的查询方法
   */
