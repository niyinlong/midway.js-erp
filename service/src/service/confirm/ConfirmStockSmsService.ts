...
/**
 * ConfirmStockSms 服务类
 * 处理 confirm_stock_sms 表的数据库操作
 */
@Provide()
export class ConfirmStockSmsService extends BaseService<ConfirmStockSms> {
  constructor(
    @InjectRepository(ConfirmStockSms)
    repository: Repository<ConfirmStockSms>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ConfirmStockSms
   * 可以在此添加业务相关的查询方法
   */
