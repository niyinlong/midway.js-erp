...
/**
 * IrActServer 服务类
 * 处理 ir_act_server 表的数据库操作
 */
@Provide()
export class IrActServerService extends BaseService<IrActServer> {
  constructor(
    @InjectRepository(IrActServer)
    repository: Repository<IrActServer>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrActServer
   * 可以在此添加业务相关的查询方法
   */
