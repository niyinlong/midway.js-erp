...
/**
 * IrActClient 服务类
 * 处理 ir_act_client 表的数据库操作
 */
@Provide()
export class IrActClientService extends BaseService<IrActClient> {
  constructor(
    @InjectRepository(IrActClient)
    repository: Repository<IrActClient>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrActClient
   * 可以在此添加业务相关的查询方法
   */
