...
/**
 * IrCronProgress 服务类
 * 处理 ir_cron_progress 表的数据库操作
 */
@Provide()
export class IrCronProgressService extends BaseService<IrCronProgress> {
  constructor(
    @InjectRepository(IrCronProgress)
    repository: Repository<IrCronProgress>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrCronProgress
   * 可以在此添加业务相关的查询方法
   */
