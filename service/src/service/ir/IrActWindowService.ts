...
/**
 * IrActWindow 服务类
 * 处理 ir_act_window 表的数据库操作
 */
@Provide()
export class IrActWindowService extends BaseService<IrActWindow> {
  constructor(
    @InjectRepository(IrActWindow)
    repository: Repository<IrActWindow>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrActWindow
   * 可以在此添加业务相关的查询方法
   */
