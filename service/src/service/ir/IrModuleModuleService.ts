...
/**
 * IrModuleModule 服务类
 * 处理 ir_module_module 表的数据库操作
 */
@Provide()
export class IrModuleModuleService extends BaseService<IrModuleModule> {
  constructor(
    @InjectRepository(IrModuleModule)
    repository: Repository<IrModuleModule>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrModuleModule
   * 可以在此添加业务相关的查询方法
   */
