...
/**
 * IrModuleCategory 服务类
 * 处理 ir_module_category 表的数据库操作
 */
@Provide()
export class IrModuleCategoryService extends BaseService<IrModuleCategory> {
  constructor(
    @InjectRepository(IrModuleCategory)
    repository: Repository<IrModuleCategory>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrModuleCategory
   * 可以在此添加业务相关的查询方法
   */
