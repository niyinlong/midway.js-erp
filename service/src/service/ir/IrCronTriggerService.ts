...
/**
 * IrCronTrigger 服务类
 * 处理 ir_cron_trigger 表的数据库操作
 */
@Provide()
export class IrCronTriggerService extends BaseService<IrCronTrigger> {
  constructor(
    @InjectRepository(IrCronTrigger)
    repository: Repository<IrCronTrigger>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrCronTrigger
   * 可以在此添加业务相关的查询方法
   */
