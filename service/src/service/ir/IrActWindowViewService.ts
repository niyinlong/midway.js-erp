...
/**
 * IrActWindowView 服务类
 * 处理 ir_act_window_view 表的数据库操作
 */
@Provide()
export class IrActWindowViewService extends BaseService<IrActWindowView> {
  constructor(
    @InjectRepository(IrActWindowView)
    repository: Repository<IrActWindowView>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrActWindowView
   * 可以在此添加业务相关的查询方法
   */
