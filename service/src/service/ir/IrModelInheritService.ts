...
/**
 * IrModelInherit 服务类
 * 处理 ir_model_inherit 表的数据库操作
 */
@Provide()
export class IrModelInheritService extends BaseService<IrModelInherit> {
  constructor(
    @InjectRepository(IrModelInherit)
    repository: Repository<IrModelInherit>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrModelInherit
   * 可以在此添加业务相关的查询方法
   */
