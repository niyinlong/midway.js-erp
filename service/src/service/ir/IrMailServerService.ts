...
/**
 * IrMailServer 服务类
 * 处理 ir_mail_server 表的数据库操作
 */
@Provide()
export class IrMailServerService extends BaseService<IrMailServer> {
  constructor(
    @InjectRepository(IrMailServer)
    repository: Repository<IrMailServer>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrMailServer
   * 可以在此添加业务相关的查询方法
   */
