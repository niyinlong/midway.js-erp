...
/**
 * IrModelAccess 服务类
 * 处理 ir_model_access 表的数据库操作
 */
@Provide()
export class IrModelAccessService extends BaseService<IrModelAccess> {
  constructor(
    @InjectRepository(IrModelAccess)
    repository: Repository<IrModelAccess>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrModelAccess
   * 可以在此添加业务相关的查询方法
   */
