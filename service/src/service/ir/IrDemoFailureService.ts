...
/**
 * IrDemoFailure 服务类
 * 处理 ir_demo_failure 表的数据库操作
 */
@Provide()
export class IrDemoFailureService extends BaseService<IrDemoFailure> {
  constructor(
    @InjectRepository(IrDemoFailure)
    repository: Repository<IrDemoFailure>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrDemoFailure
   * 可以在此添加业务相关的查询方法
   */
