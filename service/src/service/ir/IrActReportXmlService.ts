...
/**
 * IrActReportXml 服务类
 * 处理 ir_act_report_xml 表的数据库操作
 */
@Provide()
export class IrActReportXmlService extends BaseService<IrActReportXml> {
  constructor(
    @InjectRepository(IrActReportXml)
    repository: Repository<IrActReportXml>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrActReportXml
   * 可以在此添加业务相关的查询方法
   */
