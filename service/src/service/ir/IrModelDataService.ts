...
/**
 * IrModelData 服务类
 * 处理 ir_model_data 表的数据库操作
 */
@Provide()
export class IrModelDataService extends BaseService<IrModelData> {
  constructor(
    @InjectRepository(IrModelData)
    repository: Repository<IrModelData>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询IrModelData
   * 可以在此添加业务相关的查询方法
   */
