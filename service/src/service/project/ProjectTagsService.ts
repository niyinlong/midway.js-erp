...
/**
 * ProjectTags 服务类
 * 处理 project_tags 表的数据库操作
 */
@Provide()
export class ProjectTagsService extends BaseService<ProjectTags> {
  constructor(
    @InjectRepository(ProjectTags)
    repository: Repository<ProjectTags>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProjectTags
   * 可以在此添加业务相关的查询方法
   */
