...
/**
 * ProjectCollaborator 服务类
 * 处理 project_collaborator 表的数据库操作
 */
@Provide()
export class ProjectCollaboratorService extends BaseService<ProjectCollaborator> {
  constructor(
    @InjectRepository(ProjectCollaborator)
    repository: Repository<ProjectCollaborator>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProjectCollaborator
   * 可以在此添加业务相关的查询方法
   */
