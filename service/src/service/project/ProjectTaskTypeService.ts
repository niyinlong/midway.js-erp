...
/**
 * ProjectTaskType 服务类
 * 处理 project_task_type 表的数据库操作
 */
@Provide()
export class ProjectTaskTypeService extends BaseService<ProjectTaskType> {
  constructor(
    @InjectRepository(ProjectTaskType)
    repository: Repository<ProjectTaskType>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询ProjectTaskType
   * 可以在此添加业务相关的查询方法
   */
