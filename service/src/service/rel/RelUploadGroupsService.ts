...
/**
 * RelUploadGroups 服务类
 * 处理 rel_upload_groups 表的数据库操作
 */
@Provide()
export class RelUploadGroupsService extends BaseService<RelUploadGroups> {
  constructor(
    @InjectRepository(RelUploadGroups)
    repository: Repository<RelUploadGroups>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询RelUploadGroups
   * 可以在此添加业务相关的查询方法
   */
