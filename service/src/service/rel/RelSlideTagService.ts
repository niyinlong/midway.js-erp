...
/**
 * RelSlideTag 服务类
 * 处理 rel_slide_tag 表的数据库操作
 */
@Provide()
export class RelSlideTagService extends BaseService<RelSlideTag> {
  constructor(
    @InjectRepository(RelSlideTag)
    repository: Repository<RelSlideTag>
  ) {
    super(repository);
  }

  /**
   * 根据自定义条件查询RelSlideTag
   * 可以在此添加业务相关的查询方法
   */
